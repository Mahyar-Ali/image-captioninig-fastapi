...
from src.config import BATCH_SIZE, embedding_dim, model_path, top_k, units, vocab_size
...
class BahdanauAttention(tf.keras.Model):
    ...
    def call(self, features, hidden):
        ...
        score = tf.nn.tanh(self.W1(hidden_with_time_axis) + self.W2(features))
...
def load_model():
    # Loss and Optimizer
    with open(f"{model_path}/tokenizer.json") as f:
        data = json.load(f)
    ...
    c = decoder(
        tf.expand_dims([tokenizer.word_index["<start>"]] * BATCH_SIZE, 1),
        test_x,
